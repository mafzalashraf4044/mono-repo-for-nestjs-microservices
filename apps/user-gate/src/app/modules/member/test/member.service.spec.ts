...
describe('MemberService', () => {
  ...
  describe('refreshAccessTokenForUser', () => {
    it('should call refreshAccessTokenForUser with parameters', async () => {
      service.refreshMemberAccessToken(JWT_VALID_MEMBER_TOKEN);
      expect(mockedUserService.refreshAccessTokenForUser).toBeCalledTimes(1);
      expect(
        mockedUserService.refreshAccessTokenForUser.mock.lastCall[0],
      ).toEqual(JWT_VALID_MEMBER_TOKEN);
    });
  });
});
